...
import type { TallyFormControl } from '../models/tally-element';
import type TButton from '../elements/button/button';
...
//
const userInteractedControls: WeakSet<TallyFormControl> = new WeakSet();

//
// We store a WeakMap of interactions for each form control so we can track when all conditions are met for validation.
//
const interactions = new WeakMap<TallyFormControl, string[]>();

export interface FormControlControllerOptions {
  /** A function that returns the form containing the form control. */
  form: (input: TallyFormControl) => HTMLFormElement | null;
  /** A function that returns the form control's name, which will be submitted with the form data. */
  name: (input: TallyFormControl) => string;
  /** A function that returns the form control's current value. */
  value: (input: TallyFormControl) => unknown | unknown[];
  /** A function that returns the form control's default value. */
  defaultValue: (input: TallyFormControl) => unknown | unknown[];
  /** A function that returns the form control's current disabled state. If disabled, the value won't be submitted. */
  disabled: (input: TallyFormControl) => boolean;
  /**
   * A function that maps to the form control's reportValidity() function. When the control is invalid, this will
   * prevent submission and trigger the browser's constraint violation warning.
   */
  reportValidity: (input: TallyFormControl) => boolean;
  /** A function that sets the form control's value */
  setValue: (input: TallyFormControl, value: unknown) => void;
  /**
...
export class FormControlController implements ReactiveController {
  host: TallyFormControl & ReactiveControllerHost;
  form?: HTMLFormElement | null;
  options: FormControlControllerOptions;

  constructor(
    host: ReactiveControllerHost & TallyFormControl,
  ...
  private attachForm(form?: HTMLFormElement) {
    if (form) {
      console.log('attachForm', this.host, form);
      this.form = form;
      ...
      if (formCollections.has(this.form)) {
        ...
        formCollections.set(this.form, new Set<TallyFormControl>([this.host]));
      }
  ...
  private setUserInteracted(el: TallyFormControl, hasInteracted: boolean) {
    if (hasInteracted) {
  ...
  private doAction(
    type: 'submit' | 'reset',
    submitter?: HTMLInputElement | TButton
  ) {
    console.log('do it');
    if (this.form) {
  ...
  reset(submitter?: HTMLInputElement | TButton) {
    this.doAction('reset', submitter);
  }

  /** Submits the form, triggering validation and form data injection. */
  submit(submitter?: HTMLInputElement | TButton) {
  ...
  emitInvalidEvent(originalInvalidEvent?: Event) {
    const tInvalidEvent = new CustomEvent<Record<PropertyKey, never>>(
      't-invalid',
      {
        bubbles: false,
        composed: false,
    ...
    if (!originalInvalidEvent) {
      tInvalidEvent.preventDefault();
    }

    if (!this.host.dispatchEvent(tInvalidEvent)) {
